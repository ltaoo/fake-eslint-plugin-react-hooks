      ...
      onCodePathEnd: function onCodePathEnd(codePath, codePathNode) {
        ...
        try {
          for (_iterator7.s(); !(_step7 = _iterator7.n()).done;) {
            ...
            var pathsFromStartToEnd = // @ts-ignore
              ...
              for (_iterator8.s(); !(_step8 = _iterator8.n()).done;) {
                ...
                if (isDirectlyInsideComponentOrHook) {
                  ...
                  if (!cycled && pathsFromStartToEnd !== allPathsFromStartToEnd) {
                    var message = "React Hook \"".concat(context.getSource(hook), "\" is called ") + "conditionally. React Hooks must be called in the exact " + "same order in every component render." + (possiblyHasEarlyReturn ? " Did you accidentally call a React Hook after an" + " early return?" : "");
                    context.report({
                      node: hook,
                      message: message
                    });
                  ...
                  var _message = "React Hook \"".concat(context.getSource(hook), "\" cannot be called ") + "in a class component. React Hooks must be called in a " + "React function component or a custom React Hook function.";
                  ...
                  var _message2 = "React Hook \"".concat(context.getSource(hook), "\" is called in ") + "function \"".concat(context.getSource(codePathFunctionName), "\" ") + "that is neither a React function component nor a custom " + "React Hook function." + " React component names must start with an uppercase letter." + ' React Hook names must start with the word "use".';
                  ...
                  var _message3 = "React Hook \"".concat(context.getSource(hook), "\" cannot be called ") + "at the top level. React Hooks must be called in a " + "React function component or a custom React Hook function.";
                  ...
                  if (isSomewhereInsideComponentOrHook) {
                    var _message4 = "React Hook \"".concat(context.getSource(hook), "\" cannot be called ") + "inside a callback. React Hooks must be called in a " + "React function component or a custom React Hook function.";
...
var ExhaustiveDeps = {
  ...
  create: function create(context) {
    ...
    var extraSafeHooks = context.options && context.options[0] && context.options[0].safeHooks || [];
    var options = {
    ...
    function visitFunctionWithDependencies(node, declaredDependenciesNode, reactiveHook, reactiveHookName, isEffect) {
      ...
      function isStableKnownHookValue(resolved) {
        ...
        if (callee.type === "MemberExpression" && callee.property != null && !callee.computed) {
          //  force callee as ESTree.Identifier
          // @ts-ignore
        ...
        // dispatch is id, useDispatch is callee
        ...
        function groupSafeHooks(safeHooks) {
          return safeHooks.filter(function (safeHook) {
            var _safeHook = _slicedToArray(safeHook, 1),
                hookName = _safeHook[0];

            return typeof hookName === "string";
          }).map(function (safeHook) {
            var _safeHook2 = _toArray(safeHook),
                hookName = _safeHook2[0],
                extraOptions = _safeHook2.slice(1);

            return _defineProperty({}, hookName, {
              stable: extraOptions.length === 0,
              stableIdentifiers: extraOptions
            });
          }).reduce(function (total, cur) {
            return _objectSpread2(_objectSpread2({}, total), cur);
          }, {});
        }

        var groupedSafeHooks = groupSafeHooks(extraSafeHooks); // console.log('check stable', name);

        if (groupedSafeHooks[name]) {
          var _ret = function () {
            var _groupedSafeHooks$nam = groupedSafeHooks[name],
                stable = _groupedSafeHooks$nam.stable,
                stableIdentifiers = _groupedSafeHooks$nam.stableIdentifiers;
            ...
            if (id.type === "ArrayPattern" && isArray(resolved.identifiers)) {
              ...
              for (var i = 0; i < stableIdentifiers.length; i += 1) {
                var _id$elements$stableId, _id$elements$stableId2;

                var stableIdentifier = stableIdentifiers[i];
                console.log((_id$elements$stableId = id.elements[stableIdentifier]) === null || _id$elements$stableId === void 0 ? void 0 : _id$elements$stableId.name, identifier.name);

                if (((_id$elements$stableId2 = id.elements[stableIdentifier]) === null || _id$elements$stableId2 === void 0 ? void 0 : _id$elements$stableId2.name) === identifier.name) {
                  return {
                    v: true
                  };
                }
            ...
            if (id.type === "ObjectPattern" && isArray(resolved.identifiers)) {
              var _ret2 = function () {
                var identifiers = resolved.identifiers;
                var identifier = identifiers[0];

                for (var _i = 0; _i < stableIdentifiers.length; _i += 1) {
                  var matched = id.properties.find(function (property) {
                    var needCheckIsStable = stableIdentifiers.find(function (name) {
                      return [property.key.name, property.value.name].includes(name);
                    });

                    if (needCheckIsStable) {
                      return [property.key.name, property.value.name].includes(identifier.name);
                    }
                  });
        ...
        } // console.log('resolved identifiers', resolved.identifiers);
          ...
          if (id.type === "ArrayPattern" && id.elements.length === 2 && isArray(resolved.identifiers)) {
            ...
            if (id.elements[1] === resolved.identifiers[0]) {
              ...
              if (name === "useState") {
                ...
                for (var _i2 = 0; _i2 < _references.length; _i2++) {
                  stateVariables.add(_references[_i2].identifier);
                }
      ...
      function isFunctionWithoutCapturedValues(resolved) {
        ...
        for (i = 0; i < childScopes.length; i++) {
          ...
          fnNode.type === "FunctionDeclaration" && childScopeBlock === fnNode || fnNode.type === "VariableDeclarator" && childScopeBlock.parent === fnNode) {
            // Found it!
            fnScope = childScope;
            break;
          }
      ...
      function gatherDependenciesRecursively(currentScope) {
        ...
        try {
          for (_iterator.s(); !(_step = _iterator.n()).done;) {
            ...
            var dependencyNode = getDependency(referenceNode); // dependencyNode === reference.identifier
            // console.log(reference.identifier.name, dependencyNode);
      ...
      function reportStaleAssignment(writeExpr, key) {
        ...
        reportProblem({
          node: writeExpr,
          message: "Assignments to the '".concat(key, "' variable from inside React Hook ") + // @ts-ignore
          "".concat(context.getSource(reactiveHook), " will be lost after each ") + "render. To preserve the value over time, store it in a useRef " + "Hook and keep the mutable value in the '.current' property. " + "Otherwise, you can move this variable directly inside " + // @ts-ignore
          "".concat(context.getSource(reactiveHook), ".")
        });
      ...
      if (!declaredDependenciesNode) {
        ...
        if (setStateInsideEffectWithoutDeps) {
          ...
          reportProblem({
            node: reactiveHook,
            message: "React Hook ".concat(reactiveHookName, " contains a call to '").concat(setStateInsideEffectWithoutDeps, "'. ") + "Without a list of dependencies, this can lead to an infinite chain of updates. " + "To fix this, pass [" + _suggestedDependencies.join(", ") + "] as a second argument to the ".concat(reactiveHookName, " Hook."),
            suggest: [{
              desc: "Add dependencies array: [".concat(_suggestedDependencies.join(", "), "]"),
              fix: function fix(fixer) {
                return fixer.insertTextAfter(node, ", [".concat(_suggestedDependencies.join(", "), "]"));
              }
            }]
          });
        }
      ...
      if (declaredDependenciesNode.type !== "ArrayExpression") {
        ...
        reportProblem({
          ...
          message: // @ts-ignore
          "React Hook ".concat(context.getSource(reactiveHook), " was passed a ") + "dependency list that is not an array literal. This means we " + "can't statically verify whether you've passed the correct " + "dependencies."
        });
      } else {
        declaredDependenciesNode.elements.forEach(function (declaredDependencyNode) {
          ...
          if (declaredDependencyNode.type === "SpreadElement") {
            reportProblem({
              ...
              message: // @ts-ignore
              "React Hook ".concat(context.getSource(reactiveHook), " has a spread ") + "element in its dependency array. This means we can't " + "statically verify whether you've passed the " + "correct dependencies."
            });
            return;
            ...
            if (/Unsupported node type/.test(error.message)) {
              if (declaredDependencyNode.type === "Literal") {
                ...
                reportProblem({
                  node: declaredDependencyNode,
                  message: // @ts-ignore
                  "React Hook ".concat(context.getSource(reactiveHook), " has a ") + "complex expression in the dependency array. " + "Extract it to a separate variable so it can be statically checked."
                });
              }
          ...
          console.log("[declaredDependencyNode]", maybeID);

          while (maybeID.type === "MemberExpression" || // @ts-ignore
          maybeID.type === "OptionalMemberExpression" || maybeID.type === "ChainExpression") {
            // @ts-ignore
      ...
      if (problemCount === 0) {
        ...
        constructions.forEach(function (_ref5) {
          ...
          var message = "The '".concat(construction.name.name, "' ").concat(depType, " ").concat(causation, " the dependencies of ") + "".concat(reactiveHookName, " Hook (at line ").concat(declaredDependenciesNode.loc.start.line, ") ") + "change on every render. ".concat(advice);
      ...
      if (!extraWarning && missingDependencies.has("props")) {
        ...
        if (isPropsOnlyUsedInMembers) {
          extraWarning = " However, 'props' will change when *any* prop changes, so the " + "preferred fix is to destructure the 'props' object outside of " + "the ".concat(reactiveHookName, " call and refer to those specific props ") + // @ts-ignore
          "inside ".concat(context.getSource(reactiveHook), ".");
        }
      }
      ...
      if (!extraWarning && missingDependencies.size > 0) {
        ...
        missingDependencies.forEach(function (missingDep) {
          ...
          for (var _i3 = 0; _i3 < usedDep.references.length; _i3++) {
            id = usedDep.references[_i3].identifier;
      ...
      if (!extraWarning && missingDependencies.size > 0) {
        ...
        missingDependencies.forEach(function (missingDep) {
          ...
          for (var _i4 = 0; _i4 < references.length; _i4++) {
            id = references[_i4].identifier;
              ...
              if (maybeCall.type === "CallExpression") {
                ...
                if (correspondingStateVariable != null) {
                  if (correspondingStateVariable.name === missingDep) {
                    ...
                    var resolved = references[_i4].resolved;
        ...
        if (setStateRecommendation !== null) {
          switch (setStateRecommendation.form) {
            ...
            case "updater":
              extraWarning = " You can also do a functional update '".concat(setStateRecommendation.setter, "(").concat(setStateRecommendation.missingDep.substring(0, 1), " => ...)' if you only need '").concat(setStateRecommendation.missingDep, "'") + " in the '".concat(setStateRecommendation.setter, "' call.");
              break;
      ...
      reportProblem({
        node: declaredDependenciesNode,
        message: // @ts-ignore
        "React Hook ".concat(context.getSource(reactiveHook), " has ") + (getWarningMessage(missingDependencies, "a", "missing", "include") || getWarningMessage(unnecessaryDependencies, "an", "unnecessary", "exclude") || getWarningMessage(duplicateDependencies, "a", "duplicate", "omit")) + extraWarning,
        suggest: [{
    ...
    function visitCallExpression(node) {
      ...
      var notReactHookOrCustomHookNeedCheckDeps = callbackIndex === -1;

      if (notReactHookOrCustomHookNeedCheckDeps) {
      ...
      var mustHasDeps = !declaredDependenciesNode && !isEffect;
        ...
        case "Identifier":
          ...
          switch (def.node.type) {
            case "FunctionDeclaration":
              // useEffect(() => { ... }, []);
              visitFunctionWithDependencies( // @todo 不太可能出现 FunctionDeclaration
              // @ts-ignore
              def.node, declaredDependenciesNode, reactiveHook, reactiveHookName, isEffect);
              return;
            // Handled
...
function collectRecommendations(_ref8) {
  var dependencies = _ref8.dependencies,
      declaredDependencies = _ref8.declaredDependencies,
      stableDependencies = _ref8.stableDependencies,
      externalDependencies = _ref8.externalDependencies,
      isEffect = _ref8.isEffect;
  ...
  declaredDependencies.forEach(function (_ref10) {
    var key = _ref10.key;
...
function scanForConstructions(_ref11) {
  var declaredDependencies = _ref11.declaredDependencies,
      declaredDependenciesNode = _ref11.declaredDependenciesNode,
      componentScope = _ref11.componentScope,
      scope = _ref11.scope;
  var constructions = declaredDependencies.map(function (_ref12) {
    var key = _ref12.key;
  ...
  return constructions.map(function (_ref13) {
...
function getDependency(node) {
  if ( // @ts-ignore
  (node.parent.type === "MemberExpression" || // @ts-ignore
  node.parent.type === "OptionalMemberExpression") && // @ts-ignore
  node.parent.object === node && // @ts-ignore
  node.parent.property.name !== "current" && // @ts-ignore
  !node.parent.computed && !( // @ts-ignore
  node.parent.parent != null && (node.parent.parent.type === "CallExpression" || // @ts-ignore
  node.parent.parent.type === "OptionalCallExpression") && // @ts-ignore
  node.parent.parent.callee === node.parent)) {
    // @ts-ignore
    return getDependency(node.parent);
  } else if ( // Note: we don't check OptionalMemberExpression because it can't be LHS.
  // @ts-ignore
  node.type === "MemberExpression" && // @ts-ignore
  node.parent && // @ts-ignore
  node.parent.type === "AssignmentExpression" && // @ts-ignore
...
function fastFindReferenceWithParent(start, target) {
  ...
  while (queue.length) {
    ...
    for (var _i5 = 0, _Object$entries = Object.entries(item); _i5 < _Object$entries.length; _i5++) {
...
function isNodeLike(val) {
  return _typeof(val) === "object" && val !== null && !Array.isArray(val) && typeof val.type === "string";
}
